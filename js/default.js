$(document).ready(function() {
	...
	$("#regfrm").on("submit", function(event) {
		...
		N.json.register($("#regfrm").serialize(), function(obj) {
			if (obj.status == "error") {
				$("#reg-error").html(obj.message.replace(/\n/g, "<br />"));
				$("#cptxt").html("");
				N.reloadCaptcha();
			} else {
				if (obj.status == "ok") {
	...
	plist.on("click", ".closed", function(e) {
		...
		N.json[plist.data("type")].openPost({
			...
			if (m.status != "ok") {
				...
				icon.attr("class", "mdi mdi-lock-open-outline");
				me.attr("class","post-interaction opened");
			}
		});
	});
	// Opened post icon
	plist.on("click", ".opened", function(e) {
		...
		N.json[plist.data("type")].closePost({
			...
			if (m.status != "ok") {
				...
				icon.attr("class", "mdi mdi-lock-outline");
				me.attr("class","post-interaction closed");
			}
		});
	});
	plist.on("click", ".edit", function(e) {
		...
		var form = function(fid, id, message, prev, type) {
			return '<form style="margin-bottom:5px" id="' + fid + '" data-' + type + '="' + id + '">' + '<textarea id="' + fid + 'abc" autofocus style="margin: 5px; width: 98%; width: calc(100% - 10px); height:125px">' + message + "</textarea><br />" + '<input type="submit" class="btn btn-sm btn-default" value="' + N.getLangData().EDIT + '" style="float: right; margin-top:5px; margin-right: 5px;" />' + '<button type="button" class="btn btn-sm btn-default" style="float:right; margin-right: 5px; margin-top: 5px" class="preview" data-refto="#' + fid + 'abc">' + prev + "</button>" + '<button type="button" class="btn btn-sm btn-default" style="margin-left: 5px; float:left; margin-top:5px" onclick="window.open(\'/bbcode.php\')">BBCode</button><a class="btn btn-sm btn-default nerdzcrush-upload" style="margin-left: 5px; margin-top: 5px; color: #FFFFFF !important;" ><span class="hidden-xs">NerdzCrush</span> <span class="glyphicon glyphicon-paperclip" title="Upload file" aria-hidden="true"></span></a>' + "</form>";
		};
